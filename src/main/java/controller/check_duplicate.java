...
public class check_duplicate extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String a=(String)request.getParameter("email_id");
		String b="need more info";
		
model m=new model();
/* agent jj=m.getparticular_agent_byemail_status(a,b); */
agent jj=m.get_particular_agent_by_mail(a);



if(jj==null)
{
		RequestDispatcher rd=request.getRequestDispatcher("agent_registeration.jsp");
		request.setAttribute("k", a);
		rd.forward(request, response);
	
	
}else {
	String user_id=jj.getAgentId();
	System.out.println();
	agent  list=m.get_particular_agent(user_id);
	
	if(list.getStatus().equals("need more info")) {
	RequestDispatcher rd=request.getRequestDispatcher("agent_update_2.jsp");
	request.setAttribute("LIST", list);
	rd.forward(request, response);
	}else if(list.getStatus().equals("confirm")) {
		response.sendRedirect("pnf.jsp");
	}else if(list.getStatus().equals("rejected"))
	{
		RequestDispatcher rd=request.getRequestDispatcher("agent_registeration.jsp");
		request.setAttribute("k", a);
		rd.forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int j=model.update_agent_1(agentid, fullName, email, phone, licenseNumber, specialization,
                ...
                is, is1, is2);
        agent list=model.get_particular_agent(agentid);
        if(j!=0)
		{
...
//			rx.forward(request, response);
	...
	private void sendOrderConfirmationEmail(String recipientEmail,String a,String b,String c,String d,String e) {
	    ...
	    try {
	        ...
	        message.setSubject("Confirmation of Agent Update Request Received 🏡");
...
message.setContent(
	    ...
	    + "                                                    Dear " + a + ",<br><br>"
	    + "                                                    Your request to update your details has been successfully received. Once your request is reviewed and approved by our admin, we will notify you by email.<br><br>"
